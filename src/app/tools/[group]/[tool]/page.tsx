...
export default function SingleToolPage() {
  ...
  const handleStartProcessing = async (files: UploadedFile[]) => {
    ...
    try {
      ...
      // أضف أي أداة أخرى هنا بنفس الطريقة
    } catch (error) {
  ...
  const processImageToPdf = async (files: UploadedFile[]) => {
    ...
    setResultFiles([{ url: URL.createObjectURL(blob), name: "merged-result.pdf", data: new File([blob], "merged-result.pdf") }]);
    confetti({ particleCount: 150, spread: 70 });
  };
    ...
    <div className="max-w-4xl mx-auto px-6 py-12">
      ...
      {resultFiles.length === 0 ? (
        <div className={isProcessing ? "opacity-40 pointer-events-none" : ""}>
          <FileUploader key={uploaderKey} allowedTypes={["image/*"]} onProcess={handleStartProcessing} />
          {isProcessing && (
